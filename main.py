...
def redis_get(key):
    try:
        r = requests.get(f"{UPSTASH_REDIS_REST_URL}/get/{key}", headers=_redis_headers(), timeout=5)
...
def redis_set(key, value):
    try:
        requests.get(f"{UPSTASH_REDIS_REST_URL}/set/{key}/{value}", headers=_redis_headers(), timeout=5)
    except Exception as e:
        print(f"❌ Redis SET lỗi: {e}")

def redis_sismember(key, member):
    try:
        r = requests.get(f"{UPSTASH_REDIS_REST_URL}/sismember/{key}/{member}", headers=_redis_headers(), timeout=5)
...
def redis_sadd(key, member):
    try:
        requests.get(f"{UPSTASH_REDIS_REST_URL}/sadd/{key}/{member}", headers=_redis_headers(), timeout=5)
    except Exception as e:
...
def get_isod_fingerprint():
    params = {"q": "mynewsfingerprint", "username": ISOD_USERNAME, "apikey": ISOD_API_KEY}
    try:
...
def get_isod_headers():
    params = {"q": "mynewsheaders", "username": ISOD_USERNAME, "apikey": ISOD_API_KEY}
    try:
        response = requests.get(ISOD_BASE_URL, params=params, timeout=10)
        if response.status_code == 200:
...
def check_isod_notifications():
    ...
    if new_items:
        ...
        type_map = {
            "1000": "📢 Ogłoszenie", "1001": "📝 Sprawdzian", "1002": "⚠️ Ważne",
            "1003": "📊 Stan projektu", "1004": "🔄 Zmiana grupy", "1005": "✍️ Zapisy"
        }
        ...
        print("ℹ️ Fingerprint đổi nhưng không có hash mới")
